    ...
    return (
      <Layout>
        <div className="max-w-3xl mx-auto text-center py-20 space-y-4">
          ...
          <p className="text-indigo-300">Looks like you haven't added any courses yet.</p>
            ...
            {cartItems.map(item => (
              <div key={item.id} className="flex gap-4 p-4 rounded-xl bg-[#1E293B]/80 backdrop-blur-sm border border-indigo-500/20">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-indigo-300">₹{item.price}</p>
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="w-6 h-6 rounded-full border border-indigo-500/30 text-white hover:bg-indigo-500/20 transition">+</button>
                    <button onClick={() => removeItem(item.id)} className="text-sm ml-4 text-indigo-400 hover:text-indigo-300">Remove</button>
                  </div>
                </div>
                <div className="text-right">
          ...
          <div className="space-y-4">
            <div className="p-4 rounded-xl bg-[#1E293B]/80 backdrop-blur-sm border border-indigo-500/20">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between font-bold pt-2 border-t border-indigo-500/30 text-white">
                  <span>Total</span>
                  <span>₹{total}</span>
                </div>
              </div>
              ...
              <div className="mt-4 flex gap-2">
                <input
                  ...
                  value={couponCode}
                  onChange={(e) => setCouponCode(e.target.value)}
                  className="flex-1 px-3 py-2 rounded-lg border border-indigo-500/30 bg-[#0F172A] text-white placeholder:text-indigo-300/50 focus:outline-none focus:border-indigo-400"
                />
              ...
              <button
                onClick={handleCheckout}
                disabled={loading}
                className="w-full mt-4 py-2 rounded-lg font-semibold bg-indigo-600 text-white hover:bg-indigo-700 transition disabled:opacity-50"
              >
                Proceed to Checkout →
              </button>
            </div>
