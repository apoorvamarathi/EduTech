      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-4 items-center">
          <input 
            ...
            className="flex-1 p-2 rounded-lg border border-indigo-500/30 bg-[#0F172A] text-white placeholder:text-indigo-300/50 focus:outline-none focus:border-indigo-400"
          />
          <select 
            value={category} 
            onChange={e => setCategory(e.target.value)} 
            className="p-2 rounded-lg border border-indigo-500/30 bg-[#0F172A] text-white"
          >
            <option value="">All Categories</option>
            {categories.map(cat => <option key={cat} value={cat}>{cat}</option>)}
          </select>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map(course => (
            <div key={course.id} className="rounded-xl overflow-hidden bg-[#1E293B]/80 backdrop-blur-sm border border-indigo-500/20 transition hover:shadow-lg hover:shadow-indigo-500/10">
              <img src={course.thumbnail || 'https://via.placeholder.com/300x150'} alt={course.title} className="w-full h-40 object-cover" />
              <div className="p-4">
                <h3 className="text-lg font-semibold text-white">{course.title}</h3>
                <p className="text-sm text-gray-300 truncate">{course.description}</p>
                <div className="flex flex-wrap gap-1 mt-2">
                ...
                <div className="flex justify-between items-center mt-3">
                  <span className="text-lg font-bold text-indigo-400">{course.isFree ? 'Free' : `₹${course.price}`}</span>
                  <Link to={`/course/${course.id}`} className="px-3 py-1 rounded text-sm bg-indigo-600 text-white hover:bg-indigo-700 transition">View Details</Link>
                </div>
              </div>
            </div>
          ))}
        </div>
        {filtered.length === 0 && <div className="text-center py-10 text-gray-400">No courses found.</div>}
      </div>
    </Layout>
  );
}
