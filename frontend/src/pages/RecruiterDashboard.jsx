...
export default function RecruiterDashboard() {
  ...
  return (
    <Layout>
      <div className="space-y-8">
        {/* Welcome header */}
        <div className="relative overflow-hidden rounded-2xl p-6 bg-indigo-500/10 border border-indigo-500/30">
          <div className="relative z-10">
            ...
            {company ? (
              ...
              <Link to="/company-register" className="inline-block mt-2 text-sm text-indigo-400 hover:text-indigo-300 underline">+ Register your company first</Link>
            )}
          </div>
          <div className="absolute top-0 right-0 opacity-10 text-7xl">🏢</div>
        </div>
        ...
        {/* Stats cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <motion.div whileHover={{ y: -2 }} className="p-4 rounded-xl bg-[#1E293B]/80 backdrop-blur-sm border border-indigo-500/20">
          ...
          <motion.div whileHover={{ y: -2 }} className="p-4 rounded-xl bg-[#1E293B]/80 backdrop-blur-sm border border-indigo-500/20">
            ...
            <p className="text-2xl font-bold text-indigo-400">{stats.shortlisted}</p>
          </motion.div>
        </div>

        {/* Action buttons */}
        <div className="flex gap-4 flex-wrap">
          <Link to="/post-job" className="px-5 py-2 rounded-lg font-medium transition hover:scale-105 bg-indigo-600 text-white hover:bg-indigo-700">+ Post New Job</Link>
          {!company && <Link to="/company-register" className="px-5 py-2 rounded-lg border border-indigo-500/30 text-gray-200 hover:bg-indigo-500/10 transition">Register Company</Link>}
        </div>
        ...
        <div>
          ...
          {jobs.length === 0 ? (
            <div className="text-center py-12 rounded-xl bg-[#1E293B]/80 backdrop-blur-sm border border-indigo-500/20">
              ...
              <Link to="/post-job" className="inline-block mt-3 text-sm text-indigo-400 hover:text-indigo-300 underline">Post your first job</Link>
            </div>
          ) : (
            <div className="grid gap-4">
              {jobs.map(job => (
                <div key={job.id} className="flex justify-between items-center p-4 rounded-xl bg-[#1E293B]/80 backdrop-blur-sm border border-indigo-500/20">
                  <div>
                    <h3 className="font-semibold text-lg text-white">{job.title}</h3>
                    <p className="text-sm text-gray-300">{job.location} · {job.salary}</p>
                    <div className="flex flex-wrap gap-2 mt-1">
                      {job.skills.map(skill => <span key={skill} className="text-xs px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300">{skill}</span>)}
                    </div>
                    <p className="text-xs text-gray-400 mt-1">Posted: {new Date(job.postedAt).toLocaleDateString()}</p>
                  </div>
                  <div className="text-right">
                    ...
                    <Link to={`/applications/${job.id}`} className="block text-sm mt-2 text-indigo-400 hover:text-indigo-300 underline">View Applications →</Link>
                  </div>
                </div>
              ))}
            </div>
