    ...
    return (
      <Layout>
        <div className="max-w-2xl mx-auto text-center space-y-6 py-20">
          ...
          <Link to="/dashboard" className="inline-block px-6 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition">Go to Dashboard</Link>
        </div>
      </Layout>
    );
  }
  ...
  return (
    <Layout>
      <div className="max-w-2xl mx-auto text-center space-y-6 py-8">
        ...
        <p className="text-gray-300">Your transaction ID: <strong className="text-indigo-400">{payment.paymentId}</strong></p>
        <div className="p-4 rounded-xl text-left bg-[#1E293B]/80 backdrop-blur-sm border border-indigo-500/20">
          <p className="font-semibold text-white mb-2">You've purchased:</p>
          <ul className="list-disc list-inside space-y-1 text-gray-300">
            {payment.items.map(item => <li key={item.id}>{item.title} (₹{item.price})</li>)}
          </ul>
          <p className="mt-3 text-white">Total paid: <strong className="text-indigo-400">₹{payment.amount}</strong></p>
        </div>
        <div className="flex gap-4 justify-center">
          <button onClick={downloadInvoice} className="px-6 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition">Download Invoice (PDF)</button>
          <Link to="/dashboard" className="px-6 py-2 rounded-lg border border-indigo-500/30 text-gray-200 hover:bg-indigo-500/10 transition">Go to Dashboard</Link>
        </div>
      </div>
    </Layout>
  );
