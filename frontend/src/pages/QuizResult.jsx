  ...
  return (
    <Layout>
      <div className="max-w-2xl mx-auto text-center space-y-6">
        ...
        <p className="text-lg text-gray-300">Your score: <strong className="text-indigo-400">{score}%</strong> ({correct}/{totalQuestions} correct)</p>
        <p className="text-gray-300">
          {passed ? 'You passed! A certificate will be issued shortly.' : 'Passing score is 70%. Keep learning and try again.'}
        </p>
        <div className="flex gap-4 justify-center">
          <Link to="/quizzes" className="px-6 py-2 rounded-lg border border-indigo-500/30 text-gray-200 hover:bg-indigo-500/10 transition">Back to Quizzes</Link>
          {passed && <Link to="/my-certificates" className="px-6 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition">View Certificate</Link>}
        </div>
      </div>
    </Layout>
  );
