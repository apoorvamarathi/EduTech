...
export default function CertificateViewer() {
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-[#0F172A]">
      <div className="text-center text-white">Loading your certificate...</div>
    </div>
  );
  if (!cert) return (
    <div className="min-h-screen flex items-center justify-center bg-[#0F172A]">
      <div className="text-center">
        <p className="text-white">Certificate not found.</p>
        <Link to="/my-certificates" className="text-indigo-400 hover:text-indigo-300">← Back to certificates</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[#0F172A]">
      <div className="max-w-4xl w-full certificate-container print:shadow-none print:border-0">
        <div className="relative rounded-2xl shadow-2xl overflow-hidden bg-[#1E293B] border border-indigo-500/30">
          <div className="absolute inset-0 pointer-events-none border-8 border-double border-indigo-500/30 rounded-2xl"></div>

          <div className="p-8 md:p-12 text-center relative z-10">
            <h1 className="text-3xl md:text-5xl font-bold tracking-wide text-indigo-400">CERTIFICATE OF COMPLETION</h1>
            <p className="text-md md:text-lg mt-4 text-gray-300">This is to certify that</p>
            <h2 className="text-3xl md:text-5xl font-serif font-bold my-4 text-white">{cert.studentName}</h2>
            <p className="text-md md:text-lg text-gray-300">has successfully completed the course</p>
            <p className="text-2xl md:text-3xl font-semibold mt-2 text-indigo-400">{cert.courseTitle}</p>
            
            <div className="flex flex-wrap justify-center gap-8 mt-8 text-sm text-gray-300">
              <div><span className="font-semibold text-white">Grade:</span> {cert.grade}</div>
              <div><span className="font-semibold text-white">Score:</span> {cert.score}</div>
              <div><span className="font-semibold text-white">Issue Date:</span> {cert.issueDate}</div>
              <div><span className="font-semibold text-white">Credential ID:</span> {cert.credentialId}</div>
            </div>

            <div className="flex flex-col items-center mt-8">
              <img src={cert.qrCodeUrl} alt="Verification QR" className="w-28 h-28 border-2 border-indigo-500/30 rounded-lg p-1 bg-white" />
              <p className="text-xs mt-2 text-gray-400">Scan to verify authenticity</p>
            </div>

            <div className="grid grid-cols-2 gap-8 mt-10 pt-4 border-t border-indigo-500/30">
              <div className="text-left">
                <div className="w-40 h-0.5 bg-indigo-500/50 mb-1"></div>
                <p className="text-sm font-medium text-gray-200">Authorized Signature</p>
                <p className="text-xs text-gray-400">Dr. Sarah Johnson</p>
              </div>
              <div className="text-right">
                <div className="w-40 h-0.5 bg-indigo-500/50 mb-1 ml-auto"></div>
                <p className="text-sm font-medium text-gray-200">Edutech Platform</p>
                <p className="text-xs text-gray-400">www.edutech.com/verify</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-center gap-4 mt-8 no-print">
          <button
            onClick={() => window.print()}
            className="px-6 py-2 rounded-xl font-medium transition hover:scale-105 flex items-center bg-indigo-600 text-white hover:bg-indigo-700"
          >
            <PrintIcon /> Print Certificate
          </button>
          <Link
            to="/my-certificates"
            className="px-6 py-2 rounded-xl border border-indigo-500/30 font-medium text-gray-200 hover:bg-indigo-500/10 transition"
          >
            ← Back to Certificates
          </Link>
        </div>
