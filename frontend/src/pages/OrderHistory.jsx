    ...
    <Layout>
      <div className="space-y-6">
        ...
        {orders.length === 0 ? (
          ...
          <div className="space-y-4">
            {orders.map(order => (
              <div key={order.id} className="p-4 rounded-xl bg-[#1E293B]/80 backdrop-blur-sm border border-indigo-500/20">
                <div className="flex justify-between items-start flex-wrap gap-4">
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-xs px-2 py-1 rounded-full bg-indigo-500/20 text-indigo-300">{order.status}</span>
                    <button className="block text-sm mt-1 text-indigo-400 hover:text-indigo-300 underline" onClick={() => alert('Download invoice')}>Invoice PDF</button>
                  </div>
                </div>
              </div>
            ))}
